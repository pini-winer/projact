...
void delete_date(BST **root, Customers *customer)
{
    ...
    if (convert_date_to_int((*root)->customer_pointer->date) == convert_date_to_int(customer->date)){
...
int find_if_id_good(BST *root, Customers *customer)
{
    ...
    if( root->customer_pointer->id == customer->id){
        if (strcmp(second_name_temp, second_name_customer) != 0 
            || strcmp(first_name_temp, first_name_customer) != 0 )
        {
            printf("id is in use\n");
            return 1;
        }
